namespace AOC2024
{
    public class Day10A
    {
        ...
        public void Solve(List<string> data)
        {
            ...
            for (int r = 0; r < map.Count; r++)
            {
                for (int c = 0; c < map[0].Count; c++)
                {
                    ...
                    sum += FindNines((r, c), 0, map, visited);
                }
            }
        ...
        private int FindNines((int r, int c) pos, int target, List<List<int>> map, bool[,] visited)
        {
            ...
            int sum = 0;
            sum += FindNines(Move(pos, Dir.U, 1), target + 1, map, visited);
            sum += FindNines(Move(pos, Dir.R, 1), target + 1, map, visited);
            sum += FindNines(Move(pos, Dir.D, 1), target + 1, map, visited);
            sum += FindNines(Move(pos, Dir.L, 1), target + 1, map, visited);
            return sum;
        }
